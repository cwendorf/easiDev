...
plotMoments <- function(x, ...) {
  UseMethod("plotMoments")
}

plotMoments.bsm <- plotMoments.wsm <- function(x, add = FALSE, main = NULL, ylab = "Outcome", xlab = "", ylim = NULL, offset = 0, col = "black", values = TRUE, digits = 3, pos = 2, pch = 16) {
  temp <- describeMoments(x, main = main, digits = digits)
...
plotMoments.data.frame <- function(frame, add = FALSE, main = NULL, ylab = "Outcome", xlab = "", ylim = NULL, offset = 0, col = "black", values = TRUE, digits = 3, pos = 2, pch = 16) {
  results <- describeMoments(frame, main = main, digits = digits)
  .deviations(results, main = main, ylab = ylab, xlab = xlab, ylim = ylim, values = values, digits = digits, pos = pos, pch = pch, add = add, col = col, offset = offset)
  invisible(eval(frame))
}

plotMoments.formula <- function(formula, add = FALSE, main = NULL, ylab = "Outcome", xlab = "", ylim = NULL, offset = 0, col = "black", values = TRUE, digits = 3, pos = 2, pch = 16) {
  results <- describeMoments(formula, main = main, digits = digits)
  .deviations(results, main = main, ylab = ylab, xlab = xlab, ylim = ylim, values = values, digits = digits, pos = pos, pch = pch, add = add, col = col, offset = offset)
  invisible(eval(formula))
}

addMoments <- function(...) {
  plotMoments(..., add = TRUE)
}
