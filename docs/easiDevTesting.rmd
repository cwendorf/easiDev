...
graph.default <- function(x, contrast = NULL, ...) {
  howmany <- nrow(describeMoments(x))
  if (!is.null(contrast)) {
    plotSubsetsMeans(x, contrast = contrast, ...)
  } else if (howmany == 2) {
    plotComparisonMeans(x, ...)
  } else {
    plotSetMeans(x, ...)
  }
  invisible(eval(x))
...
graph.list <- function(results, add = FALSE, ...) {
  out <- results
  if (length(results) == 1) {
...
(Outcome~Factor) |> focus(Level1, Level3) |> graph()
